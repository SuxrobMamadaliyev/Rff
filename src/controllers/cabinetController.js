...
import { PAYMENT_METHOD_LABELS, ORDER_STATUS, WITHDRAWAL_STATUS } from '../utils/constants.js';
...
export const showCabinet = async (ctx) => {
  if (ctx.callbackQuery) await ctx.answerCbQuery();
  const text = messages.cabinet(ctx.state.user);
  try {
    return await ctx.editMessageText(text, { parse_mode: 'HTML', ...cabinetKeyboard() });
  } catch (_err) {
    return ctx.reply(text, { parse_mode: 'HTML', ...cabinetKeyboard() });
  }
};

export const showProfileInline = async (ctx) => {
  await ctx.answerCbQuery();
  return ctx.reply(messages.profile(ctx.state.user), { parse_mode: 'HTML' });
};
...
export const showWithdrawalHistory = async (ctx) => {
  await ctx.answerCbQuery();
  const withdrawals = await Withdrawal.find({ telegramId: ctx.from.id }).sort({ createdAt: -1 }).limit(10);
